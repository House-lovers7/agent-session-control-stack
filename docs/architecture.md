...
(a) Health 層だけが「畳む」判断を出す、(b) Checkpoint/Recovery 層は「畳む」イベントの前後にのみ反応する。
...
- Health: transcript メトリクス相当が無いため、代理シグナル（経過時間 / tool call 数 / diff 量 / テスト失敗回数）+ 自己申告 checkpoint。wrapper による自動監視は Phase 2 以降
