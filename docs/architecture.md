...
- Claude Codeではhooks / plugin / local proxyとして、Codexではnative compact hooks + AGENTS.md + state directoryとして、同一の4層パターンを実装する
...
flowchart TB
  ...
  P -. "Codex binding" .-> Codex["native compact hooks +\nAGENTS.md + .agent-session/"]
  R -. "Codex binding + fallback" .-> Codex
```
...
│    - summary is hypothesis,                │
│      state is untrusted; source is truth   │
└────────────────────────────────────────────┘
```
...
### 3.3 Checkpoint — compact-plus（SessionStart + PreCompact）

- SessionStart hook: 後続のstate lifecycleで使うsession identifierをexport
...
| SessionStart | — | session identifier export | 関与しない |
| リクエスト経路 | — | — | proxy として圧縮 |
...
## 7. Codex binding（native hook + portable fallback）

現行Codexの `PreCompact` / `PostCompact` / `SessionStart(source=compact)` をnative boundaryとして使い、AGENTS.md + `.agent-session/` をstate本文の更新契約とfallbackにする:

- Compression: 上流 0.8.0 は OpenAI Responses transport を実装するが、ASCS から Codex へのrouting・認証・tool挙動・byte-exact safetyはend-to-end未検証。stable bindingには移植せず、明示的なedge実験だけに限定する（risk-register参照）
...
- Checkpoint: `PreCompact(manual|auto)`がtranscript本文を保存せず、既存state file名と境界metadataだけをreceiptへ記録。state本文はAGENTS.md protocolで更新する
- Recovery: `PostCompact`でreceiptを閉じ、`SessionStart(source=compact)`が同一sessionへ1回だけ検査・再検証ルールを追加する。hook無効時はhandoff.mdから手動再開する

詳細は [adapter-interface.md](adapter-interface.md) と [codex/agents-md-draft.md](codex/agents-md-draft.md)。

## 8. 事実と推測の区分

- [高] §3 の各ツール仕様: [immutable upstream lock](../config/upstreams.lock.json) の README / manifest / hooks / transport source を 2026-07-10 に直接確認
- [中] 3 ツール併用で長時間セッションの安定性・コスト効率が上がる — 各層の個別実測はあるが、**併用時の相乗・相互作用は未測定**
- [高] Codex hook event、matcher、trust/disable条件は[公式Hooks仕様](https://learn.chatgpt.com/docs/hooks)を2026-07-16に確認
- [中] Codex native bindingの実効性 — focused testはあるが、manual/auto compactの実runtime dispatchとstate遵守率は未検証
